...
//void GameScene::UpdateMatrixChild(int idx, int idxParent)
//{
//	// スケーリング行列を宣言
//	Matrix4 matScale;
//	// スケーリング倍率を行列に設定する
...
//	worldTransforms_[idx].matWorld_ *= worldTransforms_[idxParent].matWorld_;
//
//	// 行列の転送
...
void GameScene::Update() {
	...
	// キャラクター移動処理
	{
		...
		for (int i = 0; i < kNumPartId; i++) {
			UpdateMatrix(worldTransforms_[i]);
		}
	}
